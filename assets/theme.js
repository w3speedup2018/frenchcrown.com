...
//                       $('.buy-now-1').hide();
      ...
      value: function _onOptionChanged(newValue, target, activator) {
        ...
        // Make to Order message display
        var sku = $('.variant_list option:selected').attr('data-sku').split('-');
        var selected_size = sku[sku.length-1].split('/')[0];
        if($('.mto-sizes').length > 0 && jQuery.inArray(selected_size, $('.mto-sizes').text().split(',')) >= 0){
          $('.make-to-order-msg:first, .expected-delivery-date:first').show();
        }else{
          $('.make-to-order-msg, .expected-delivery-date').hide();
        }
...
//         document.dispatchEvent(new CustomEvent('theme:loading:start'));
        ...
        if($('.ProductForm__Item').length > 0 && !$('.sticky-bottom').hasClass('show')){
          ...
          var blazer_size = '';
          if(meta.product.type == 'Shirt' || meta.product.type == 'Designer Shirt'){
...
//            check variant is available or not
          ...
          }else if(meta.product.type == 'Sherwani Pajama' || meta.product.type == 'Jodhpuri Pajama' || meta.product.type == 'Indowestern Pajama' || meta.product.type == 'Wedding kurta Pajama' || meta.product.type == 'Sherwani Dhoti' || meta.product.type == 'Jodhpuri Dhoti' || meta.product.type == 'Indowestern Dhoti' || meta.product.type == 'Wedding kurta Dhoti' || meta.product.type == 'Wedding Kurta Koti Lounge Pant'|| meta.product.type == 'Wedding Kurta Koti Churi Pant'|| meta.product.type == 'Wedding Kurta Indowestern Lounge Pant'|| meta.product.type == 'Wedding Kurta Indowestern Churi Pant'|| meta.product.type == 'Wedding Kurta Sherwani Lounge Pant'|| meta.product.type == 'Wedding Kurta Sherwani Churi Pant'|| meta.product.type == 'Indowestern Lounge Pant' || meta.product.type == 'Indowestern Churi Pant'){
            var pant_size = '';
          	$('.ProductForm__Item').each(function(index){
              if(!$(this).hasClass('option-selected')){
                is_size_selected = false;
				if($('.size-error').length < 1){
                  ...
                  errorMessageElement.className = 'ProductForm__Error Alert Alert--error';
                  if(index == 0){
                  	errorMessageElement.innerHTML = '<p class="size-error">Please select Top Garment Size</p>';
                  }else{
                  	errorMessageElement.innerHTML = '<p class="size-error">Please select Bottom Size</p>';
                  }
...
//            check variant is available or not
              ...
              if($.inArray( selected_variant, title_array ) == -1){
                ...
                if($('.size-error').length < 1){
                  var errorMessageElement = document.createElement('span');
                  errorMessageElement.className = 'ProductForm__Error Alert Alert--error';
                  errorMessageElement.innerHTML = '<p class="size-error">Your bottom size is not accurate according to your top garment size. please change your bottom size.</p>';
                  addToCartButton.setAttribute('disabled', 'disabled');
                  addToCartButton.insertAdjacentElement('afterend', errorMessageElement);
